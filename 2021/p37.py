        ...
        for i,s2 in regularized.items():
            ...
            for p1 in scanner_[:-11]:  # twelve HAVE to overlap. after checking all but 11, give up.
                for p2 in s2[:-11]:
                    ...
                    if len(ov) >= 12:
                        return apply_move(scanner_, -relative)
    for i in regularized:
        checked.add((i, index))
        checked.add((index, i))
    return None
...
def check_scanners(scanners):
    ...
    while len(regularized) < len(scanners):
        ...
        for i, scanner in enumerate(scanners):
            if i in regularized:
                continue
            print(f"Checking {i}")
            scanner_ = check_against_regularized(scanner, regularized, turns, checked, i)
            if scanner_:
    ...
    return len(set().union(*(set(map(hash, scanner)) for scanner in regularized.values())))
